...
static esp_err_t _open_door_get_handler(httpd_req_t *req);
static esp_err_t _open_door_post_handler(httpd_req_t *req);
static esp_err_t _close_door_get_handler(httpd_req_t *req);
static esp_err_t _close_door_post_handler(httpd_req_t *req);

static esp_err_t _status_get_handler(httpd_req_t *req);
...
static esp_err_t _status_get_handler(httpd_req_t *req)
{
  ...
  const char *p_json_resp = application_get_json(NULL);
  httpd_resp_set_status(req, HTTPD_200);
  httpd_resp_set_type(req, "application/json");
  // httpd_resp_set_hdr( req, "Connection", "keep-alive" );

  httpd_resp_send(req, p_json_resp, strlen(p_json_resp));
...
</style>\n<center><h1>" DEVICE_NAME "</h1></center>\
...
static esp_err_t _ota_post_handler(httpd_req_t *req)
{
  ...
  printf("Writing partition: type %d, subtype %d\n", update_partition->type, update_partition->subtype);
  printf("Running partition: type %d, subtype %d\n", running->type, running->subtype);
...
void http_start_webserver(httpd_handle_t *p_server)
{
  ...
  if (httpd_start(p_server, &config) == ESP_OK)
  {
    ...
    static httpd_uri_t openDoor_post =
        {
            .uri = "/openDoor",
            .method = HTTP_POST,
            .handler = _open_door_post_handler,
            .user_ctx = NULL,
        };
    httpd_register_uri_handler(*p_server, &openDoor_post);

    static httpd_uri_t closeDoor_post =
        {
            .uri = "/closeDoor",
            .method = HTTP_POST,
            .handler = _close_door_post_handler,
            .user_ctx = NULL,
        };
    httpd_register_uri_handler(*p_server, &closeDoor_post);

    static httpd_uri_t openDoor_get =
        {
            .uri = "/openDoor",
            .method = HTTP_GET,
            .handler = _open_door_get_handler,
            .user_ctx = NULL,
        };
    httpd_register_uri_handler(*p_server, &openDoor_get);

    static httpd_uri_t closeDoor_get =
        {
            .uri = "/closeDoor",
            .method = HTTP_GET,
            .handler = _close_door_get_handler,
            .user_ctx = NULL,
        };
    httpd_register_uri_handler(*p_server, &closeDoor_get);
    ...
    httpd_register_uri_handler(*p_server, &status_get);
